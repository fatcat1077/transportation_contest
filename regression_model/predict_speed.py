...
def load_data(counts_path, speed_path):
    """讀取 counts 與 speed 原始 CSV，合併並產生 target（5 分鐘後速度） 回傳含 interval/large/small/target 的 DataFrame."""
...
def main():
    p = argparse.ArgumentParser(description="5-min ahead speed prediction with various regressors")
    p.add_argument("--counts", help="Path to counts CSV")
    p.add_argument("--speed", help="Path to speed CSV")
    p.add_argument("--model", default="speed_model.pkl", help="Model file path")
    p.add_argument("--train", action="store_true", help="Incremental train")
    p.add_argument(
        "--regressor", choices=["linear", "gbr", "svr", "knn"], default="linear", help="Which regressor to use"
    )
    p.add_argument("--predict", nargs=2, type=float, metavar=("LARGE", "SMALL"), help="Single prediction")
    p.add_argument("--input-csv", help="CSV for batch prediction")
    p.add_argument("--output-csv", help="Save batch predictions to CSV")
    args = p.parse_args()
    ...
    if args.input_csv and args.output_csv:
        ...
        df["predicted_speed"] = df.apply(lambda r: predict_speed(args.model, r["large"], r["small"]), axis=1)
