...
def download_for_date(day:date, hours:list[str]):
    ...
    for hr in hours:
        ...
        for fn in links:
            ...
            try:
                rr=requests.get(url+fn,timeout=20); rr.raise_for_status()
...
def summarize_day(day_str:str, hours:list[str]) -> pd.DataFrame|None:
    ...
    for hr in hours:
        ...
        for fp in fld.glob("TDCS_M05A_*.csv"):
            ...
            df=pd.read_csv(fp,header=None,skiprows=1,usecols=[1,2,3,4,5],
                           names=["s1","s2","veh","speed","vol"],dtype=str)
    ...
    wide=long.pivot_table(index=["date","time_5min"],columns="veh",
                          values=["speed","vol"],aggfunc={"speed":"mean","vol":"sum"})
...
def export_and_plot(wide:pd.DataFrame,day_str:str,
                    start_t:time,end_t:time,is_today:bool,root:tk.Tk):
    # 0) 計算各車種密度 & LOS
    for veh in VEHICLES.values():
        vol=wide[f"{veh}_流量"]*12
        wide[f"{veh}_密度"]=vol/ wide[f"{veh}_車速"]
        wide[f"{veh}_LOS"]=pd.cut(wide[f"{veh}_密度"],bins=LOS_BREAKS,
                                   labels=LOS_LABELS,right=False)
    # PCU 加權整體
    pcu5,spdpcu=[],[]
    for veh,pcu in PCU.items():
        if f"{veh}_流量" in wide:
            wide[f"{veh}_PCU5"]=wide[f"{veh}_流量"]*pcu; pcu5.append(f"{veh}_PCU5")
            wide[f"{veh}_SPD×PCU"]=wide[f"{veh}_車速"]*wide[f"{veh}_PCU5"]; spdpcu.append(f"{veh}_SPD×PCU")
    ...
    # 2) 篩時段
    if is_today:
        now=datetime.now().time(); end_t=min(end_t,now)
    ...
    ax.set_xticks(idxs); ax.set_xticklabels(times,rotation=45,ha="right")
    ...
    for x,y,l in zip(idxs, sub["total_flow_rate"], los_all):
        ax2.text(x, y+sub["total_flow_rate"].max()*0.03, l,
...
class App(tk.Tk):
    ...
    def run_today(self):
        def job():
            now=datetime.now()
            day_dt=date.today(); ds=day_dt.strftime("%Y%m%d")
            ...
            if wide is None: return self.after(0,lambda:messagebox.showerror("錯誤","無資料"))
            st=(now-timedelta(hours=2)).time(); et=now.time()
