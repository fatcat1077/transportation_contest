...
def process_labels(label_dir):
    ...
    for txt_path in glob.glob(os.path.join(label_dir, "*.txt")):
        ...
        df = pd.read_csv(txt_path, sep=r"\s+", header=None, names=["class", "x", "y", "w", "h"])
        ...
        records.append({"interval": interval, "large": n_large, "small": n_small})
...
def main():
    parser = argparse.ArgumentParser(description="統計每 5 分鐘的大／小車數量並輸出 CSV")
    parser.add_argument("--label_dir", required=True, help="存放 YOLO 標注 txt 檔案的資料夾路徑")
    parser.add_argument("--output", default="vehicle_counts.csv", help="輸出的 CSV 檔名 (預設: vehicle_counts.csv)")
